import { useState } from "react"
import ProductCard from "./product-card"


function BrandCard({name, number, brandNumb}) {
  const [productsState, setProductsState] = useState(false);

  return (
    <div className="brand-card">
        <div className={productsState? "brand-cTittle bctOnn" : "brand-cTittle"} onClick={()=>setProductsState(!productsState)}>
            <p className="font3">{name}</p>
            <svg  viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.29289 5.29289C8.68342 4.90237 9.31643 4.90237 9.70696 5.29289L15.707 11.2929C16.0975 11.6834 16.0975 12.3164 15.707 12.707L9.70696 18.707C9.31643 19.0975 8.68342 19.0975 8.29289 18.707C7.90237 18.3164 7.90237 17.6834 8.29289 17.2929L13.5859 11.9999L8.29289 6.70696C7.90237 6.31643 7.90237 5.68342 8.29289 5.29289Z" fill="white"/>
            </svg>
        </div>

        <div className={productsState? "product-list-on" : "product-list-off"}>
        {[...Array(number)].map((_, index) => (
          <ProductCard key={index} brandNumb={brandNumb} indexPro={index}/>
        ))}
        </div>

    </div>
  )
}

export default BrandCard
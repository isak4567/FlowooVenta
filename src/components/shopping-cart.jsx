...
function ShoppingCart() {
  const {orderList, setOrderList, amountProd} = useContext(orderContext);
  const [windWidth, setWindWidth] = useState(screen.width);
  const [fullCart, setFullCart] = useState("shopping-cart");


  //useEffect(() => (setWindWidth(screen.width)),[])

  const carroComprars = (e, mode) => {
    e.stopPropagation();

    if (orderList.length > 0 && mode == 1) {

      // Aqui para para hacer algo con la informcion de la orden de la persona
      console.log(orderList);
    } else if (orderList.length > 0) {
      if (fullCart == "shopping-cart") {
        setFullCart("shopping-cart sc-Full");
        setWindWidth(800);
      }
      else {
        setFullCart("shopping-cart");
        setWindWidth(screen.width);
      }
    }
  };
  
  return (
    <div className={fullCart} onClick={(e)=>carroComprars(e,2)}>

      <div className={windWidth > 780 && orderList.length > 0? "sc-costList" : "no-disp"}>
      ...
      <div className={windWidth < 780? "sc-pl-movil" : "no-disp"}>
        <p className="font3">Productos: {amountProd}</p>
        <p className="font3">Total: $PrecioTotalMXN</p>
      </div>

      <button className={orderList.length > 0? "sc-button" : "sc-button-off"} onClick={(e)=> carroComprars(e, 1)}>
